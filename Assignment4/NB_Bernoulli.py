...
def summarize(dataset):
    summaries = [np.mean(attribute) for attribute in zip(*dataset)]
    del summaries[-1]
    return summaries
...
def main():
    ...
    for i in dataset_k_split:
        ...
        train_accuracy = calculate_accuracy(predict(training_x, mean_dict, bern_dict, prob_zero_y, prob_one_y), training_y)
