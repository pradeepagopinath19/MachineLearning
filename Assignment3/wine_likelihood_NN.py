# Citation - https://deepnotes.io/softmax-crossentropy
...
class NeuralNetwork:
    ...
    def feedforward(self):
        ...
        self.output = self.softmax(np.add(np.dot(self.layer1, self.weights2), self.b2))

    def calculate_loss(self):
        self.loss = self.cross_entropy(self.output, self.y)

    def softmax_stable(self, X):
        exps = np.exp(X - np.max(X))
        return exps / np.sum(exps)

    def softmax(selfself, X):
        exps = np.exp(X)
        return exps / np.sum(exps, axis=1, keepdims=True)

    def cross_entropy(self, pred, y):
        return np.sum(-y * np.log(pred))/len(y)
...
def run_neural_networks():
    ...
    for _ in range(epochs):
        ...
        # print(nn.y)
        print(nn.loss)
        error_matrix = (np.subtract(nn.output, nn.y))/len(nn.output)
        #print(error_matrix)
        error_hidden = error_matrix.dot(nn.weights2.T)
        ...
        nn.weights2 -= nn.layer1.T.dot(error_matrix) * learning_rate
        nn.weights1 -= nn.input.T.dot(d_hidden) * learning_rate

        # updating biases

        nn.b1 -= np.sum(a=d_hidden, axis=0, keepdims=True) * learning_rate
        nn.b2 -= np.sum(a=error_matrix, axis=0, keepdims=True) * learning_rate
